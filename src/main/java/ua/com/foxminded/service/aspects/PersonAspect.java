...
public class PersonAspect {
    ...
    void beforeCreateAdvice(JoinPoint joinPoint) {
        ...
        try {
            if (person == null) {
                IllegalArgumentException exception = new IllegalArgumentException("A person can't be null when create.");
            ...
            if (personId != 0) {
                ...
                LOGGER.error("A person {} has wrong id {} which is not equal zero when create.", person, personId, exception);
                throw exception;
            }
    ...
    void beforeUpdateStudentAndLecturerAdvice(JoinPoint joinPoint) {
        ...
        try {
            ...
            if (personId < 1) {
                IllegalArgumentException exception = new IllegalArgumentException("A person id isn't positive for existing object.");
                LOGGER.error("An updated person {} has wrong id {} which is not positive.", person, personId, exception);
                throw exception;
            }
        } catch (IllegalArgumentException illegalArgumentException) {
